// Tratamento de eventos XML (Inutilização, Cancelamento, etc.).
// Funções estão documentadas em português.
...
export function editarInutilizacao(
  ...
  if (scenario.editar_emitente && emitenteMap) {
    ...
    if (cnpjTag && "CNPJ" in emitenteMap) {
      cnpjNovo = emitenteMap["CNPJ"];
  ...
  if (scenario.editar_data && scenario.nova_data) {
    const novaData = new Date(
      scenario.nova_data.split("/").reverse().join("-")
    );
    ...
    const dhRecbtoTag = helper.findElementDeep(
      root,
...
export function editarCancelamento(
  ...
  if (
    chNFeTag &&
    chNFeTag.textContent &&
    chNFeTag.textContent in chaveMapping
  ) {
  ...
  if (editarData && novaData) {
    const novaDh = `${
      new Date(novaData.split("/").reverse().join("-"))
        .toISOString()
        .split("T")[0]
    }T${new Date().toLocaleTimeString("en-GB", { hour12: false })}-03:00`;
  ...
  if (
    chNFeRet &&
    chNFeRet.textContent &&
    chNFeRet.textContent in chaveMapping
  ) {
