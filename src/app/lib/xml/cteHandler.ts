...
export function editarCte(
  ...
  if ((scenario.editar_data && scenario.nova_data) || (scenario.alterar_cUF && scenario.novo_cUF) && idAtual) {
    ...
    const aammNovo = scenario.nova_data 
      ? new Date(scenario.nova_data.split('/').reverse().join('-')).toLocaleString('en-GB', { year: '2-digit', month: '2-digit' }).replace('/', '')
      : aammAtual;
  ...
  if (ide) {
    if (scenario.alterar_cUF && scenario.novo_cUF) {
      ...
      if (tag) { tag.textContent = scenario.novo_cUF; alteracoes.push('CTe Identificação: <cUF> alterado'); altered = true; }
    }
    if (scenario.editar_data && scenario.nova_data) {
      const dhEmiTag = helper.findElement(ide, 'dhEmi');
      if (dhEmiTag) {
        const novaDh = `${new Date(scenario.nova_data.split('/').reverse().join('-')).toISOString().split('T')[0]}T${new Date().toLocaleTimeString('en-GB', { hour12: false })}-03:00`;
  ...
  if (infDoc) {
    ...
    if (chaveTag) {
      ...
      if (chaveTag.textContent && chaveTag.textContent in chaveMapping) {
        ...
        alteracoes.push(`Referência de NFe <chave> atualizada para: ${chaveTag.textContent}`);
        updated = true;
      } else if (chaveDaVendaNova && chaveTag.textContent !== chaveDaVendaNova) {
        chaveTag.textContent = chaveDaVendaNova;
        alteracoes.push(`Referência de NFe <chave> FORÇADA para a chave da venda: ${chaveDaVendaNova}`);
        updated = true;
      }
