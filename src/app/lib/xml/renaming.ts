// Utilitários para extrair informações e renomear arquivos XML (NFe/CTe).
// Os nomes principais permanecem em inglês por compatibilidade com o
// código existente. Foram adicionados aliases em português para uso
// em novos trechos de código — consulte as funções `obterInfoXml`,
// `obterInfoEvento` e `renomearArquivoPorRegras` como aliases.
...
export function getXmlInfo(helper: XmlHelper): Record<string, any> | null {
  try {
    ...
    if (
      root.tagName.includes("procEventoNFe") ||
      root.tagName.toLowerCase().includes("cte")
    )
      return null;
    ...
    return {
      ...
      x_texto:
        helper.findElementDeep(infNFe, "infAdic/obsCont/xTexto")?.textContent ||
        "",
      chave,
...
// Alias em português
export function obterInfoXml(helper: XmlHelper): Record<string, any> | null {
  return getXmlInfo(helper);
}
...
export function getEventoInfo(helper: XmlHelper): Record<string, any> | null {
  try {
    ...
    const tpEvento = helper.findElementDeep(
      root,
      "evento/infEvento/tpEvento"
    )?.textContent;
    if (tpEvento !== "110111") return null; // Apenas Cancelamento por enquanto

    const chaveCancelada =
      helper.findElementDeep(root, "evento/infEvento/chNFe")?.textContent ||
      null;
...
// Alias em português
export function obterInfoEvento(helper: XmlHelper): Record<string, any> | null {
  return getEventoInfo(helper);
}
...
export function renameFileAccordingToRules(info: Record<string, any>): string {
  ...
  if (REMESSAS_CFOP.includes(cfop)) {
    if (ref_nfe) {
      return `${nfe_number} - Remessa simbólica da venda ${ref_nfe
        .slice(25, 34)
        .replace(/^0+/, "")}.xml`;
    }
...
// Alias em português
export function renomearArquivoPorRegras(info: Record<string, any>): string {
