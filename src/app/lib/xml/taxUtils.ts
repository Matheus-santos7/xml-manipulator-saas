...
// Utilitários de imposto e criação de blocos IBSCBS.
// Mantém nomes de funções em português e comentários explicativos.
...
export function criarBlocoIBSCBS(
  ...
  if (!vProdTag || !vProdTag.textContent) {
    return {
      vBC: new Decimal(0),
      vIBSUF: new Decimal(0),
      vIBSMun: new Decimal(0),
      vCBS: new Decimal(0),
      vDevTrib: new Decimal(0),
    };
  }
  ...
  const createSubGroup = (
    ...
    if (aplicarReducao && gRedConfig) {
      ...
      helper.createAndSetText(
        gRed,
        "pAliqEfet",
        gRedConfig.pAliqEfet || "0.00"
      );
...
export function recalculaTotaisIpi(
  helper: XmlHelper,
  infNFe: Element,
  alteracoes: string[]
) {
  ...
  let somas = {
    ...
    vIPI: new Decimal(0),
    vDesc: new Decimal(0),
    vFrete: new Decimal(0),
    vSeg: new Decimal(0),
    vOutro: new Decimal(0),
  };

  const safeGetDecimal = (
    element: Element | null,
    tagName: string
  ): Decimal => {
    const tag = helper.findElement(element, tagName);
    return tag && tag.textContent
      ? new Decimal(tag.textContent)
      : new Decimal(0);
  };
  ...
  for (const det of dets) {
    ...
    if (ipiTag) {
      somas.vIPI = somas.vIPI.add(
        safeGetDecimal(helper.findElementDeep(ipiTag, "IPITrib/vIPI"), "vIPI")
      );
    }
  }

  const novoVnf = Object.values(somas)
